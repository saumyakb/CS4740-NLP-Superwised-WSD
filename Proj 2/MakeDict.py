...
def nb(ws,wf,s,w,pf,af,bf,alfa2):
    pws={}
    for num in s:
        try:
            pws[num] = (float(ws[num][w]+alfa)/float(s[num]+len(wf)*alfa))*(float(s[num])/total)
            #print pws[num]
        except:
            pws[num] = (float(alfa)/float(len(wf)*alfa))*(float(s[num])/total)
            #print pws[num]
...
test(word_sense,word_freq,sense,pos_feature,alfa2=beta)
